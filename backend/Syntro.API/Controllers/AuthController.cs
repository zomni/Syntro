...
public class AuthController : Controller
{
    ...
    private const string RememberMeClaimType = "syntro:remember_me";
    private const string CanManageUsersClaimType = "syntro:can_manage_users";
    private const string MfaModeClaimType = "syntro:mfa_mode";
    private const string MfaSetupKeyClaimType = "syntro:mfa_setup_key";
    private const string MfaUserIdClaimType = "syntro:mfa_user_id";
    private const string MfaReturnUrlClaimType = "syntro:mfa_return_url";
    ...
    public async Task<IActionResult> MfaSetup(MfaSetupViewModel model, CancellationToken cancellationToken)
    {
        ...
        if (!success)
        {
            ModelState.AddModelError(string.Empty, "El codigo MFA no es valido. Verifica que estes usando la entrada nueva de Syntro/admin y que la hora del telefono este sincronizada automaticamente.");
            var session = _mfaService.GetEnrollmentSession(pending.SetupKey);
            if (session is not null)
            {
                model.Username = pending.Username;
    ...
    public IActionResult Session()
    {
        ...
        var sites = new[]
        {
            new { campusKey = "sotero", name = "Sotero" }
        };
