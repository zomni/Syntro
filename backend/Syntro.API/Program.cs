    ...
    .AddCookie(options =>
    {
        ...
        options.Events = new CookieAuthenticationEvents
        {
            OnRedirectToLogin = context =>
            {
                ...
                var publicPath = context.Request.Headers.TryGetValue("X-Syntro-Public-Path", out var value)
                    ? value.ToString()
                    : string.Empty;
...
builder.Services.AddAuthentication()
    .AddCookie("MfaPending", options =>
    {
        ...
        options.Cookie.Name = "Syntro.MfaPending";
        options.Cookie.HttpOnly = true;
...
builder.Services.AddSingleton<PackageRestoreProgressStore>();
...
builder.Services.AddScoped<RoomSuggestionService>();
...
app.Use(async (context, next) =>
{
    if (context.Request.Path.StartsWithSegments("/dashboard", out var dashboardRemaining))
    {
        ...
        context.Request.Headers["X-Syntro-Public-Path"] = context.Request.Path.ToString();
