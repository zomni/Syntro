...
public class FrontendSyncService
{
    ...
    public async Task<FrontendSyncResult> SyncAsync(CancellationToken cancellationToken = default)
    {
        ...
        var catalogPath = Path.Combine(dataRoot, "syntro_buildings_catalog.json");

        if (!File.Exists(catalogPath))
        {
            throw new FileNotFoundException("No se encontró syntro_buildings_catalog.json en la ruta configurada.", catalogPath);
        }
    ...
    public string ResolveDataRoot()
    {
        ...
        return Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "syntro", "src", "data"));
    }
