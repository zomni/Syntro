...
{
    private const string CachePrefix = "syntro:mfa:setup:";
        ...
        IConfiguration configuration)
    {
        _context = context;
        _cache = cache;
        _protector = dataProtectionProvider.CreateProtector("Syntro.API.MfaSecret.v1");
    ...
    public bool IsRequiredForRole(string role)
    {
        ...
        return normalizedRole is AppRoles.Admin
    ...
    public async Task<MfaSetupSession?> BeginEnrollmentAsync(AuthUser user, CancellationToken cancellationToken = default)
    {
        ...
        var issuer = GetString("MfaSettings:Issuer", "MFA_ISSUER") ?? "Syntro";
