"""
Migrate inventory data from sotero_map_api to Syntro.

Copies:
  - New ImportedInventoryItems (GLPI+TOSHIBA source) not yet in Syntro
...
SYNTRO_DB = r'C:\Users\paolo.vilches\Documents\repos\Syntro\backend\data\syntro.db'
...
SYNTRO_INVENTORY_FORMS = r'C:\Users\paolo.vilches\Documents\repos\Syntro\backend\data\inventory-forms'
SYNTRO_INVENTORY_DOCS = r'C:\Users\paolo.vilches\Documents\repos\Syntro\backend\data\inventory-documents'
...
def main():
    print('=== Sotero -> Syntro Inventory Migration ===')
    print()

    sotero = sqlite3.connect(SOTERO_DB)
    syntro = sqlite3.connect(SYNTRO_DB)
    sc = sotero.cursor()
    pc = syntro.cursor()

    now = datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')

    # 1. Build mapping: (SourceFile, RowNumber) -> Syntro Guid Id
    pc.execute('SELECT Id, SourceFile, RowNumber, DeliveryFormPdfFileName FROM ImportedInventoryItems')
    syntro_item_map = {}
    syntro_items_by_key = {}
    for row in pc.fetchall():
        syntro_item_map[(row[1], row[2])] = row[0]
        syntro_items_by_key[(row[1], row[2])] = row
    print(f'Syntro existing items: {len(syntro_item_map)}')

    # 2. Find sotero items missing in Syntro
    ...
    for row in sc.fetchall():
        ...
        if key in syntro_item_map:
            # Update DeliveryFormPdfFileName if sotero has it and Syntro doesn't
            syntro_id = syntro_item_map[key]
            syntro_pdf = syntro_items_by_key[key][3]  # index 3 = DeliveryFormPdfFileName
            if item['DeliveryFormPdfFileName'] and item['DeliveryFormPdfFileName'] != syntro_pdf:
                pdf_updates.append((syntro_id, item['DeliveryFormPdfFileName']))
        else:
    ...
    # 3. Insert new items
    if new_items:
        sotero_id_to_syntro_id = {}
        # Columns shared between sotero and Syntro (excluding sotero-only 'Id')
        shared_cols = [c for c in sotero_cols if c != 'Id']
        # Add OrgId and default values for Syntro-only columns
        ...
        for item in new_items:
            syntro_id = new_guid()
            key = (item['SourceFile'], item['RowNumber'])
            syntro_item_map[key] = syntro_id
            sotero_id_to_syntro_id[item['Id']] = syntro_id

            values = []
            for col in shared_cols:
                values.append(item[col])
            # Syntro-only defaults
            values.extend([
                SOTERO_ORG_ID,  # OrgId
                'rule',         # CategorySource
    ...
    # Build full sotero_id -> syntro_id map (for existing + new)
    full_id_map = {}
    for key, syntro_id in syntro_item_map.items():
        # Find sotero Id for this key
        for sotero_id, (skey, _) in all_sotero_items.items():
            if skey == key:
                full_id_map[sotero_id] = syntro_id
                break

    # 4. Update DeliveryFormPdfFileName
    for syntro_id, pdf_name in pdf_updates:
        pc.execute(
    ...
    for row in sc.fetchall():
        ...
        syntro_item_id = full_id_map.get(sotero_item_id)
        if syntro_item_id is None:
            skipped_docs += 1
            continue

        syntro_doc_id = new_guid()
        pc.execute(
            '''INSERT INTO InventoryDocuments
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, '', '', '', 0, 1)''',
            (syntro_doc_id, syntro_item_id, doc['OriginalFileName'], doc['StoredFileName'],
    ...
    # 6. Copy PDF files
    os.makedirs(SYNTRO_INVENTORY_DOCS, exist_ok=True)
    os.makedirs(SYNTRO_INVENTORY_FORMS, exist_ok=True)
    ...
    if os.path.isdir(SOTERO_INVENTORY_DOCS):
        for fname in os.listdir(SOTERO_INVENTORY_DOCS):
            ...
            dst = os.path.join(SYNTRO_INVENTORY_DOCS, fname)
    ...
    if os.path.isdir(SOTERO_INVENTORY_FORMS):
        for fname in os.listdir(SOTERO_INVENTORY_FORMS):
            ...
            dst = os.path.join(SYNTRO_INVENTORY_FORMS, fname)
            if os.path.isfile(src) and not os.path.exists(dst):
                shutil.copy2(src, dst)
    ...
    print(f'Total items in Syntro: {pc.fetchone()[0]}')
    pc.execute('SELECT COUNT(*) FROM InventoryDocuments')
    print(f'Total documents in Syntro: {pc.fetchone()[0]}')
